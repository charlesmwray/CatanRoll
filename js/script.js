    ...
    function callback(){
      return function(){
        ...
        if (t == 6 || t == 8) {
          ...
          var earthquakeText = "";
          console.log(tremorCount);
          if (tremorCount !=2) { 
            earthquakeText = "RUMBLE RUMBLE";
            tremorCount++;
            $('.earthquake').html(earthquakeText).show().delay(2000).queue(function() {
              $(this).hide();
              $(this).dequeue();
            });
          } else {
            earthquakeText = "EARTHQUAKE";
            tremorCount = 0;
            var wh = $(document).height();
            $('.earthquake')
              .html(earthquakeText)
              .append('<br><button onclick="hideEQ()" id"hideEQ">Catan Rebuilt</button>')
              .addClass('earthquakeNoAnimation')
              .css({fontSize:'150px'})
              .show();
          }
...
function undoRoll() {
  rolls.pop();
  $(".dice").html("--").removeClass('robber red');
  $("#rollCount").html(rolls.length);
  count(rolls);
}

function hideEQ() {
  $('.earthquake').fadeOut(300).css({fontSize:'100px'}).removeClass('earthquakeNoAnimation');
  $('#hideEQ').destroy();
}

//undo button
//enter names
//rules for 3 7s in a row
